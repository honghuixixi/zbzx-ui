<template>
  <div>
    <Statistics :active="2"/>
    <div class="title">
      <span>当前位置 : 管理平台 /<span class="currentTable"> 消息</span></span>
      <span>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      //      跳转到详情页
      goInfo(id){
        this.$router.push({path: '/page/newsinfo', query:{id: id}});
      },
      //      分页方法
...
//      发布消息
      goPublish(){
        this.$router.push({path: '/page/writenews'});
      }
    },
    mounted () {
      this.$nextTick(() => {
