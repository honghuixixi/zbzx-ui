<template>
  <div id="consulInfo">
    ...
    <!--弹窗end-->
    <div class="title">
      <span><span style="cursor: pointer" @click="goBack">当前位置 : 管理平台 /</span><span class="currentTable"> 技术咨询详情</span></span>
      <span>
    ...
    <div class="navTitleInfo">
      ...
      <span class="tag" v-if="list.type==1">采购计划</span>
      <span class="tag" v-if="list.type==2">验收管理</span>
      <span class="tag" v-if="list.type==3">使用管理</span>
      <span class="tag" v-if="list.type==4">巡检质控</span>
      <span class="tag" v-if="list.type==5">信息化</span>
      <span class="tag" v-if="list.type==6">其它</span>
    </div>
    <div class="titleInfo">
      <span v-if="list.publisher"><span>申请人：{{list.publisher}}</span><span class="org" v-if="list.phone">手机号：{{list.phone}}</span><span class="org" v-if="list.email" style="word-break: break-all">邮箱：{{list.email　|orgLong}}</span><span style="display: inline-block;width: 20px"></span><span v-if="list.wx">微信号：{{list.wx}}</span><br>
        <span >机构信息：{{list.org | list}}</span><span class="org">提交时间：{{list.publishTime | timeChange}}</span><br>
        <span >产品：{{list.productor |list}}</span><span class="org">厂商：{{list.manufacturer | list}}</span>
      </span>
    </div>
...
<script>
  ...
  import {getConsulInfo,delectDetailend} from '@/service/api'
  import {getConsulcomment} from '@/service/api'
  import {subConsulcomment} from '@/service/api'
...
//      获取咨询回复
      getConsulcomment (index) {
        let param = {
          id:this.$route.query.id,
          pageNo: index,
          pageSize: this.page.pageSize
        }
        getConsulcomment(param, (data) => {
...
//      提交咨询事件
      subCommont(){
        if(this.content) {
          ...
          subConsulcomment(param, (data) => {
//          成功之后再掉回复接口，并且提示
            this.$message({
              message: '回复成功！',
              type: 'success'
            });
//          this.getConsulcomment(1)
            this.$router.go(0)
          },(msg,data)=>{
            if(data.code=='d_003'){
              this.$message.warning(msg)
